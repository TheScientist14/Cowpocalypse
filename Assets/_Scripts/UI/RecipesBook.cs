...
public class RecipesBook : MonoBehaviour
{
    ...
    private void ShowItemData(Page iPage, ItemData iItemData)
    {
        ...
        foreach(KeyValuePair<ItemData, int> recipeItemCount in iItemData.Recipes)
        {
            ...
            RessourceUI ui = ressource.GetComponent<RessourceUI>();
            ui.ItemData = recipeItemCount.Key;
            ui.UpdateValue(max: recipeItemCount.Value);
            Button button = ressource.GetComponent<Button>();
            button.onClick.AddListener(() => GoToItemPage(recipeItemCount.Key));
        }
        ...
        foreach(KeyValuePair<ItemData, int> itemUsageCount in m_ItemsUsages[iItemData])
        {
            ...
            RessourceUI ui = ressource.GetComponent<RessourceUI>();
            ui.ItemData = itemUsageCount.Key;
            ui.UpdateValue(max: itemUsageCount.Value);
            Button button = ressource.GetComponent<Button>();
            button.onClick.AddListener(() => GoToItemPage(itemUsageCount.Key));
        }
    }
