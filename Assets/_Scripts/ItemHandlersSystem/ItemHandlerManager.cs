...
public class ItemHandlerManager : Singleton<ItemHandlerManager>
{
	...
	public UnityEvent OnItemHandlerPlacement;
	...
	protected override void Awake()
	{
		...
		if(OnBeltModeChange == null)
			OnBeltModeChange = new UnityEvent();
		if(OnItemHandlerPlacement == null)
			OnItemHandlerPlacement = new UnityEvent();
	}
	...
	private void SpawnBelts()
	{
		...
		bool hasPlacedOne = false;
		hasPlacedOne |= (SpawnBelt(firstGridPos, beltAngle) != null);
		if(firstGridPos != secondGridPos)
		{
			for(Vector3Int beltGridPos = firstGridPos; beltGridPos - delta != secondGridPos; beltGridPos += delta)
				hasPlacedOne |= (SpawnBelt(beltGridPos, beltAngle) != null);
		}

		if(hasPlacedOne)
			OnItemHandlerPlacement.Invoke();
	}
	...
	private void DeleteMachineOnCursor()
	{
		...
		if(GridManager.instance.SetItemHandlerAt(new Vector2Int(cellPos.x, cellPos.y), null, true))
			OnItemHandlerPlacement.Invoke();
	}
